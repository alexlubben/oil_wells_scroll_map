...
function handleWheel(e) {
  // Only handle wheel events that aren't on the scrollbar area
  const scrollbarWidth = window.innerWidth - document.documentElement.clientWidth;
  const isOnScrollbar = e.clientX > window.innerWidth - scrollbarWidth - 20; // 20px buffer
  
  if (!isOnScrollbar && e.deltaY > 0) {
...
function handleTouchStart(e) {
  if (e.touches.length === 1) {
    // Only handle touch events not near the scrollbar area
    const scrollbarWidth = window.innerWidth - document.documentElement.clientWidth;
    const isNearScrollbar = e.touches[0].clientX > window.innerWidth - scrollbarWidth - 30;
